...
function limpiarCampos() {
    ...
    // Limpiar los campos de producto y cantidad adicionales
    let productosAdicionales = document.getElementById('productos-adicionales');
    if (productosAdicionales) productosAdicionales.innerHTML = '';
}

function actualizarNumerosProducto() {
    let productos = document.querySelectorAll('.producto-adicional');
    for (let i = 1; i < productos.length; i++) { // Comenzar desde el segundo campo de producto
        productos[i].previousSibling.textContent = 'Producto #' + (i + 1) + ':';
    }
}

function showAddProductFields() {
    // Mostrar los campos de entrada para añadir un producto
    document.getElementById('addProductFields').style.display = 'block';
    // Ocultar los campos de entrada para eliminar un producto
    document.getElementById('removeProductField').style.display = 'none';
}

function showRemoveProductField() {
    // Ocultar los campos de entrada para añadir un producto
    document.getElementById('addProductFields').style.display = 'none';
    // Mostrar los campos de entrada para eliminar un producto
    document.getElementById('removeProductField').style.display = 'block';
}
